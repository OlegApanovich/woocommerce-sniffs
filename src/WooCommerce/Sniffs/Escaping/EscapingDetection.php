<?php
/**
 * Sniff to improve escaping detection.
 */

namespace WooCommerce\Sniffs\Escaping;
...
class EscapingDetection implements Sniff
{
    ...
    private function isEscaped(File $phpcsFile, $varIndex) {
        ...
        $escapers = [
            ...
            'wp_kses_post', 'esc_js', 'esc_textarea', 'do_shortcode'
        ];

        // Check if variable is passed as argument to a function like do_shortcode($var)
        $openParen = $phpcsFile->findNext(T_OPEN_PARENTHESIS, $func + 1, null, false, null, true);
        if ($openParen !== false && $openParen < $varIndex) {
            $closeParen = $tokens[$openParen]['parenthesis_closer'] ?? null;
            if ($closeParen !== null && $varIndex < $closeParen) {
